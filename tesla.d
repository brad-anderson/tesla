...
enum refuse_topic_regex = ctRegex!(r"^[@!]topic", "gi");
...
class IrcBot
{
	...
	try
	{
		...
		if(command == "PRIVMSG")
		{
		    if(match(parameters, refuse_topic_regex))
		    {
			channelMessage(target, "No.");
		    }
			...
			foreach(url; urls)
			{
				...
				if(title_match)
				{
				    ...
				    channelAction(target, format("[ %s ]", title));
				}
				else
				{
...
void main()
{
	...
	{
		writeln("Some exception happend...recklessly ignoring");
	}
    }
}
